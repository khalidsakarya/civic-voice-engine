...
async function main() {
  ...
  // Print the header rows to understand column structure
  console.log('Header rows (1-5):');
  rows.slice(0, 5).forEach((r, i) => {
    console.log(`Row ${i+1} (${r.length} cells):`);
    r.forEach((c, j) => { if (c) console.log(`  [${j}]: ${c}`); });
  });

  // Print last 10 rows (most recent data)
  console.log('\nLast 10 rows:');
  rows.slice(-10).forEach((r, i) => {
    console.log(`Row ${rows.length - 10 + i + 1}:`, r.slice(0, 20).join(' | '));
  });

  console.log('\nTotal rows:', rows.length);
  console.log('Max columns:', Math.max(...rows.map(r => r.length)));
}
