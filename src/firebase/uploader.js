...
/**
 * Upload member committee assignments (CA ourcommons.ca, US clerk.house.gov,
 * UK members-api, AU aph.gov.au) to the `member_committees` Firestore collection.
 * Reads all JSON files from output/member_committees/.
 */
async function uploadMemberCommittees() {
  const files = loadLatestFiles(path.join(OUTPUT_ROOT, 'member_committees'));
  const allRecords = [];
  for (const file of files) {
    const { records } = JSON.parse(fs.readFileSync(file));
    allRecords.push(...records.map(withTimestamp));
 ...
 * Reads all JSON files from output/member_expenses/.
 */
async function uploadMemberExpenses() {
  const files = loadLatestFiles(path.join(OUTPUT_ROOT, 'member_expenses'));
  const allRecords = [];
  for (const file of files) {
    const { records } = JSON.parse(fs.readFileSync(file));
    allRecords.push(...records.map(withTimestamp));
  ...
  if (allRecords.length === 0) {
    console.log('[firebase] ⚠ member_expenses: no records found, skipping');
    return 0;
  }
  const count = await batchWrite('member_expenses', allRecords);
  console.log(`[firebase] ✓ member_expenses: ${count} documents written`);
  return count;
}

/**
 * Upload member biographies (CA openparliament.ca, US congress.gov, UK members-api,
 * AU aph.gov.au HTML scrape) to the `member_bios` Firestore collection.
 * Reads all JSON files from output/member_bios/.
 */
async function uploadMemberBios() {
  const files = loadLatestFiles(path.join(OUTPUT_ROOT, 'member_bios'));
  const allRecords = [];
  for (const file of files) {
    const { records } = JSON.parse(fs.readFileSync(file));
    allRecords.push(...records.map(withTimestamp));
  ...
  if (allRecords.length === 0) {
    console.log('[firebase] ⚠ member_bios: no records found, skipping');
    return 0;
  }
  const count = await batchWrite('member_bios', allRecords);
  console.log(`[firebase] ✓ member_bios: ${count} documents written`);
  return count;
}
 ...
 */
async function uploadMemberVotes() {
  const files = loadLatestFiles(path.join(OUTPUT_ROOT, 'member_votes'));
  const allRecords = [];
  for (const file of files) {
    const { records } = JSON.parse(fs.readFileSync(file));
    allRecords.push(...records.map(withTimestamp));
  ...
  if (allRecords.length === 0) {
    console.log('[firebase] ⚠ member_votes: no records found, skipping');
    return 0;
  }
  const count = await batchWrite('member_votes', allRecords);
  console.log(`[firebase] ✓ member_votes: ${count} documents written`);
  return count;
}
...
const CANONICAL_STAT_DEFS = {
  ...
  medianGrossRent: {
    ...
    UK: { source: 'Office for National Statistics — IPHRP Index of Private Housing Rental Prices, series L522 (2015=100)',
          sourceUrl: 'https://www.ons.gov.uk/economy/inflationandpriceindices/timeseries/l522/data',
          liveKey: 'medianGrossRent' },
